    ...
    <main className="min-h-screen">
      <header className="py-36 bg-indigo-50 relative">
        <div className="container text-center text-foreground">
          <h1 className="text-3xl md:text-6xl font-bold">All Subjects</h1>
          <ul className="block font-normal text-lg">
            ...
            <li className="inline-block">All Subjects</li>
          </ul>
        </div>
      </header>
        ...
        <div className="w-full mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {
            data?.categories.map((category:any)=>{
                return (<HoverCard key={category?.id}>
      ...
      <HoverCardContent className="w-80">
        <div className="flex flex-col justify-between space-x-4">
          <div className="space-y-1">
            ...
            {
              category?.subjectSet.map((subject:any)=>{
            return (
            <div key={subject?.standard?.id} className="flex flex-col pt-2">
              <div className="flex justify-between mt-2">
                <Link href={`/class/${subject?.standard?.slug}`}><h4 className="font-black">{subject?.standard?.name}</h4></Link>
                <div className="flex flex-col justify-evenly items-center">
                {
                  ((
                    ...
                    (
                    <>
                <h4 className="font-semibold text-primary">₹{subject?.standard?.latestPrice} <span className="line-through decoration-1 text-gray-500 text-[0.9rem] ml-1">/ ₹{subject?.standard?.beforePrice}</span></h4>
                <Badge className="bg-secondary/10 text-secondary hover:bg-secondary/50" variant={"secondary"}>{
                Math.round(((subject?.standard?.beforePrice - subject?.standard?.latestPrice)/(subject?.standard?.beforePrice))*100)
                }% OFF
                </Badge></>
                )
                :
                <h4 className="font-semibold text-primary">₹{subject?.standard?.latestPrice}</h4>)
                }
                </div>
              </div>
            </div>)
