...
export default async function MyProfilePage({ }: Props) {
  ...
  return (
    <div className="container py-4">
      <div className="relative mt-4 h-96 w-full overflow-hidden rounded-xl bg-cover bg-center">
          ...
          <div className="mb-6 flex items-center justify-between gap-6">
            <div className="flex items-center gap-6">
              <div className="avatar">
                <div className="w-32 h-32 overflow-hidden rounded-full shadow-lg shadow-blue-gray-500/40">
                  {data.me?.photo ? (
                    <img
                      src={data.me.photo}
                      width="78px"
              ...
              <div>
                <h5 className="block antialiased tracking-normal font-serif text-2xl font-semibold leading-snug text-blue-gray-900">
                  {data.me?.fullName}
                </h5>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 mb-10 gap-10 px-4 lg:grid-cols-1">
              ...
              <div className="p-0">
                ...
                <ul className="flex flex-col gap-4 p-0">
                  <li className="flex items-center gap-4">
                    <p className="block antialiased font-serif text-lg leading-normal text-blue-gray-900 font-semibold capitalize">
                      First Name:
                    </p>
                    <p className="block antialiased font-serif text-lg leading-normal font-normal text-gray-400">
                      {data.me?.fullName}
                    </p>
                  </li>
                  <li className="flex items-center gap-4">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4">
              {data.me?.enrollmentSet.map((enrolled: any) => (
                <div key={enrolled.id} className="flex flex-col card shadow p-4 gap-1 rounded-xl border border-slate-200">
                  <span className="text-xl font-bold">{enrolled.standard?.name}</span>
